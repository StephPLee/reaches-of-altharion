...
CREATE INDEX capstones_title_idx
  ON capstones (LOWER(title), id);

DROP TRIGGER IF EXISTS set_capstones_updated_at ON capstones;
CREATE TRIGGER set_capstones_updated_at
BEFORE UPDATE ON capstones
FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

ALTER TABLE homebrew_automation_entries
ADD COLUMN IF NOT EXISTS capstone_id BIGINT REFERENCES capstones(id) ON DELETE CASCADE;
...
CHECK (
  anchor_mode IN ('heading', 'link', 'item', 'guild_upgrade', 'starting_grace', 'boon', 'capstone')
);
...
CHECK (
  ...
  OR (
    anchor_mode = 'capstone'
    AND capstone_id IS NOT NULL
    AND homebrew_section_item_id IS NULL
    AND guild_upgrade_id IS NULL
    AND starting_grace_id IS NULL
    AND boon_id IS NULL
