...
If you are at 0 hit points at the start of your turn, you can expend and roll up to five Hit Dice, adding your Constitution modifier to each roll, and regaining hit points equal to the total. You can also use this feature if you have died within the last 6 seconds. You can use this feature once per day.',
  0,
  true
),
(
...
Whenever you take damage from a hostile creature, you gain one stack of Prowess Insulted. Each stack grants a +1 bonus to attack rolls, saving throw DCs, and damage rolls. All stacks of Prowess Insulted are lost at the end of your turn.',
  0,
  true
),
(
...
### Battle Master, Legally Distinct(TM)
...
When you make a melee attack, you can choose to target any number of creatures within your melee reach with the same attack. Until the start of your next turn, attack rolls against you have advantage.
...
Fall damage for you is calculated in d12s and has no limit.',
  0,
  true
),
(
...
As an action, you conjure a miniature black hole in a location you can see within 300 feet of you. Any nonmagical terrain within 30 feet around the black hole is instantly destroyed, leaving only empty air. Whenever a creature of your choice starts or ends its turn within 30 feet of the black hole, it takes 8d8 force damage.

Whenever a creature of your choice is targeted by a ranged attack while within 30 feet of the black hole, you can choose to give the attack disadvantage (no action required). As an action or bonus action on subsequent turns, you can choose to move a black hole up to 150 feet in any direction (this does not destroy nonmagical terrain). If the black hole collides with another black hole, they both explode, triggering Supernova. Any black hole created this way dissipates harmlessly after 1 minute.

### Supernova

As an action, you can cause your black hole created by **Endless Horizon** to explode violently. Doing so causes any creature within 30 feet of the black hole to make a Strength, Dexterity, Constitution, and Wisdom save. A creature takes 10d12 force damage on a failed save, and half as much on a success (this is repeated for each save).

### Wormhole Travel

Whenever you are subjected to the damage from Supernova, you can choose to gain immunity to that damage instead. Once on your turn you can, as a free action, teleport to any location within 30 feet of a black hole you have created with **Endless Horizon**.',
  0,
  true
),
(
...
WITH boon_targets AS (
  SELECT id, slug
  FROM boons
  WHERE slug IN ('psionophage', 'firebrand', 'edict-of-mercy-and-ruin')
),
deleted_setup AS (
  ...
  WHERE automation_entry_id IN (
    ...
    WHERE boon_id IN (SELECT id FROM boon_targets)
  )
),
deleted_blocks AS (
...
WITH psionophage_automation AS (
  ...
  SELECT
    id,
    'boon',
    'Psionophage Avrae Automation',
    'Expand to view setup and download options',
    0
  FROM boons
  WHERE slug = 'psionophage'
  RETURNING id
),
firebrand_automation AS (
  INSERT INTO homebrew_automation_entries (
    boon_id,
    anchor_mode,
    panel_title,
  ...
  SELECT
    id,
    'boon',
    'Firebrand Avrae Automation',
    'Expand to view setup and download options',
    0
  FROM boons
  WHERE slug = 'firebrand'
  RETURNING id
),
edict_automation AS (
  INSERT INTO homebrew_automation_entries (
  ...
  SELECT
    id,
    'boon',
    'Edict of Mercy and Ruin Avrae Automation',
    'Expand to view setup and download options',
    0
  FROM boons
  WHERE slug = 'edict-of-mercy-and-ruin'
  RETURNING id
)
...
SELECT id, 'Required Snippet', '!snippet blud -dtype pyschic>bludgeoning', 0
FROM psionophage_automation
UNION ALL
SELECT id, 'Required Snippet', '!snippet pier -dtype pyschic>piercing', 1
FROM psionophage_automation
UNION ALL
SELECT id, 'Required Snippet', '!snippet slash -dtype pyschic>slashing', 2
FROM psionophage_automation
UNION ALL
SELECT id, 'Required Snippet', '!snippet psyblud -dtype bludgeoning>psychic', 3
FROM psionophage_automation
UNION ALL
SELECT id, 'Required Snippet', '!snippet psypier -dtype piercing>psychic', 4
FROM psionophage_automation
UNION ALL
SELECT id, 'Required Snippet', '!snippet judgement -d 2d10[radiant] -f "Judgement Infusion|At the start of your turn while the storm is active, you can choose to forgo calling down bolts. Until the end of your turn, whenever you hit with an attack, you can choose for the attack to deal an additional 2d10 radiant damage, or deal no damage and instead restore 4d10 hit points to the target."', 0
FROM edict_automation;

WITH psionophage_automation AS (
  SELECT id FROM homebrew_automation_entries
  WHERE boon_id = (SELECT id FROM boons WHERE slug = 'psionophage')
),
firebrand_automation AS (
  SELECT id FROM homebrew_automation_entries
  WHERE boon_id = (SELECT id FROM boons WHERE slug = 'firebrand')
),
edict_automation AS (
  SELECT id FROM homebrew_automation_entries
  WHERE boon_id = (SELECT id FROM boons WHERE slug = 'edict-of-mercy-and-ruin')
)
...
SELECT
  id,
  'Firebrand',
  $$!a import name: Firebrand Detonation
automation:
  - type: counter
    counter: dummy
    amount: "1"
...
proper: false$$,
  'firebrand.txt',
  0
FROM firebrand_automation
UNION ALL
SELECT
  id,
  'Divine Storm - Ruin',
  $$!a import name: Divine Storm - Ruin
automation:
  - type: target
    target: all
    effects:
...
SELECT
  id,
  'Divine Storm - Mercy',
  $$!a import name: Divine Storm - Mercy
automation:
  - type: target
    target: all
    effects:
