...
- `/sync-level-roles` lets staff reconcile every linked player's bracket roles and privately reports the changes made.
...
- `/sync-level-roles` is gated by `REQUIRED_ROLE_ID`. It checks every Discord user linked to a WestMarches.games character, adds and removes bracket roles to match all of their active approved character levels, and returns an ephemeral summary with total and per-bracket changes. Level 20 grants both Master and Paragon.
